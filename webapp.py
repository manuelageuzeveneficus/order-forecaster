...
datum = st.date_input(
    "**Datum**", vandaag, min_value=vandaag, max_value=datetime.date(2023, 12, 31)
)
gem_neerslag = int(orders_maand[orders_maand.maand == datum.month].iloc[0, 1])
neerslag = st.number_input(
    "**De totale neerslag van regen, hagel en sneeuw (mm)** :rain_cloud:",
    min_value=0,
    step=1,
    value=gem_neerslag,
...
# scoring_uri = "http://34930671-f250-48e3-87e0-7cf2aa758b9b.westeurope.azurecontainer.io/score"
# headers = {"Content-Type": "application/json"}
# # resp = find out how to trigger your api with the requests package and valid data
# body = "2023-10-6" {'date': datum.strftime("%Y-%m-%d"),'precipitation': neerslag}
...
# Voorspelling
st.markdown("**Voorspelde aantal orders:**")
st.text(int(round(prediction, 0)))

# %% GIF toevoegen

st.markdown("![Alt Text](https://media.giphy.com/media/ICnE90q1JUH0poH7aB/giphy.gif)")
